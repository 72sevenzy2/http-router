...
Built for educational reasons and solely to use myself since I like having control over the tools I use.
...
- middlewares. I added about 4 middlewares in this project, the first being a simple logging middleware, second and third being authentication, (second is an BearerToken auth, which uses a token to verify a users credentials), (third being basicAuth auth, which involves a username and password to verify credentials.), and fourth being a recoverer middleware (if used make sure you use it FIRST before any other middleware), it prevents server crashes when a bug is occured.
